...
describe('retroactive byes (real Postgres): a player enrolled mid-season is backfilled for rounds before they joined', () => {
  ...
  it('the throwaway backfill-byes catch-up endpoint repairs an enrollment that predates this feature, and is a no-op on a second run', async () => {
    const names = ['CatchupAlice', 'CatchupBob'];
    const playerIds: number[] = [];
    for (const name of names) {
      const res = await app.inject({ method: 'POST', url: '/api/admin/players', headers: { cookie }, payload: { name } });
      playerIds.push(res.json().id);
    ...
    const seasonRes = await app.inject({
      ...
      headers: { cookie },
      payload: {
        name: 'Retro Bye Catchup Test Season',
        topValue: 105,
        roster: [
          { playerId: alice, startingValue: 105 },
          { playerId: bob, startingValue: 104 },
    ...
    const catchupSeasonId = seasonRes.json().id;

    const round1 = await app.inject({
      method: 'POST',
      url: `/api/admin/seasons/${catchupSeasonId}/rounds`,
      headers: { cookie },
      payload: { number: 1, date: '2026-04-01', signedUpPlayerIds: [alice, bob] },
    });
    const round1Id = round1.json().id;
    const game = round1.json().entries.find((e: { kind: string }) => e.kind === 'GAME');
    await app.inject({
      method: 'PATCH',
    ...
    // Simulate the exact real-world gap: a player enrolled through a path
    // that bypasses enrollNewOrReturningPlayer/enrollExistingPlayer
    // entirely (a raw enrollment row, same shape createSeason's own roster
    // bulk-insert produces) — no backfill ever ran for them, same as
    // anyone enrolled before this feature existed.
    const carol = await prisma.player.create({ data: { name: 'CatchupCarol', membershipType: 'GUEST' } });
    createdPlayerIds.push(carol.id);
    await prisma.seasonEnrollment.create({ data: { seasonId: catchupSeasonId, playerId: carol.id, startingValue: 100 } });

    const beforeRound1 = await app.inject({ method: 'GET', url: `/api/admin/rounds/${round1Id}`, headers: { cookie } });
    expect(beforeRound1.json().entries.some((e: { soloPlayerId: number | null }) => e.soloPlayerId === carol.id)).toBe(false);

    const backfillRes = await app.inject({
      method: 'POST',
      url: `/api/admin/seasons/${catchupSeasonId}/backfill-byes`,
      headers: { cookie },
    });
    expect(backfillRes.statusCode).toBe(200);

    const afterRound1 = await app.inject({ method: 'GET', url: `/api/admin/rounds/${round1Id}`, headers: { cookie } });
    const carolEntries = afterRound1.json().entries.filter((e: { soloPlayerId: number | null }) => e.soloPlayerId === carol.id);
    expect(carolEntries).toHaveLength(1);
    expect(carolEntries[0].kind).toBe('REGULAR_BYE');

    // Running it again must not create a second bye for the same round.
    await app.inject({ method: 'POST', url: `/api/admin/seasons/${catchupSeasonId}/backfill-byes`, headers: { cookie } });
    const afterSecondRun = await app.inject({ method: 'GET', url: `/api/admin/rounds/${round1Id}`, headers: { cookie } });
    expect(
      afterSecondRun.json().entries.filter((e: { soloPlayerId: number | null }) => e.soloPlayerId === carol.id),
    ).toHaveLength(1);

    await prisma.season.delete({ where: { id: catchupSeasonId } }).catch(() => {});
  });
});
