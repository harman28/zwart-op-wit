 ...
 * just the ones actually marked external, plus an autocomplete to add more,
 * mirroring "Who's playing?" / "Assign opponent" rather than a picklist.
 */
    ...
    <div className="external-section">
      ...
      {externalEntries.map((entry) => (
        <div className="external-row" key={entry.id}>
          ...
          <div className="external-row-controls">
            ...
            {adminMode && editingId !== entry.id && (
              <button className="x" onClick={() => onRemove(entry)} aria-label="Remove">
                ✕
              </button>
            )}
          </div>
      ...
      {adminMode && (
        <div style={{ marginTop: externalEntries.length ? 10 : 0 }}>
          <PlayerAutocomplete
            ...
            placeholder="Add a player who played externally…"
          />
        </div>
      )}
    </div>
